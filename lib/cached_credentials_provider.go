...
type SessionTokenProvider interface {
	...
	AssumeRole(d *time.Duration) (credentials.Value, error)
}
...
func (p *awsAssumeRoleProvider) validateRoleDuration(d *time.Duration) {
	p.assumeRoleDuration = *d
