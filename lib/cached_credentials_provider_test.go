...
// with nil profile
// with nil opts
// with profile.Name set
// with profile.Name unset, but profile.SourceProfile set
// test IsExpired() true & false
...
func TestNewCachedCredentialsProvider(t *testing.T) {
	...
	t.Run("SourceProfileSet", func(t *testing.T) {
		...
		if !strings.HasSuffix(p.cacher.CacheFile(), ".aws_assume_role_source") {
			t.Errorf("Unexpected value for cache file name with profile name set: %s", p.cacher.CacheFile())
		}
	})
	...
	t.Run("SourceProfileUnset", func(t *testing.T) {
		...
		p := NewCachedCredentialsProvider(prof, opts)
		if !strings.HasSuffix(p.cacher.CacheFile(), ".aws_assume_role_mock") {
			t.Errorf("Unexpected value for cache file name with profile name set: %s", p.cacher.CacheFile())
		}
	})
}
