...
func TestNewKeycloakSamlClient(t *testing.T) {
	...
	t.Run("good", func(t *testing.T) {
		u := fmt.Sprintf("%s/auth/realms/master/protocol/saml/descriptor", s.URL)
		...
		if c.realm != "master" || c.clientId != "account" {
...
func newKeycloakClient(s *httptest.Server) (*keycloakSamlClient, error) {
	...
	u, err := url.Parse(s.URL + "/auth/realms/" + realm)
	if err != nil {
		return nil, err
	}
...
func mockKeycloakHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path == "/auth/realms/master/protocol/saml/descriptor" {
		body := `
...
`
		fmt.Fprintf(w, body, r.Host, r.Host)
	} else if r.URL.Path == "/auth/realms/master/protocol/openid-connect/auth" {
		body := `
<html>
<head></head>
<body>
  <form method="post" action="http://%s/auth/realms/master/login-actions/authenticate"></form>
</body>
</html>
`
...
`
		fmt.Fprintf(w, body, r.Host)
		return
	} else if r.URL.Path == "/auth/realms/master/protocol/saml" {
		body := `
<html>
<head></head>
<body>
