...
func init() {
	...
	mux.HandleFunc("/api/2/mfa/users/", olMfaDeviceHandler)
	mux.HandleFunc("/api/2/saml_assertion", olSamlAssertionHandler)
	mux.HandleFunc("/api/2/saml_assertion/verify_factor", olSamlVerifyMfaHandler)
...
func TestOneloginClient_SamlAssertion_CodeMfa(t *testing.T) {
	t.Run("explicit configuration", func(t *testing.T) {
		c := newMockOneloginClient()
		c.Username = "samlcodemfa"
		c.Password = "goodPassword"
		c.MfaType = "code"
		c.MfaTokenProvider = func() (string, error) {
			return "54321", nil
...
func TestOneloginClient_SamlAssertion_PushMfa(t *testing.T) {
	t.Run("explicit configuration", func(t *testing.T) {
		c := newMockOneloginClient()
		c.Username = "samlpushmfa"
		c.Password = "goodPassword"
		c.MfaType = "push"
...
func olSamlAssertionHandler(w http.ResponseWriter, r *http.Request) {
	...
	if params["password"] != "goodPassword" {
		data, _ := json.Marshal(&oneloginApiErrorV2{Status: http.StatusUnauthorized, Message: "Authentication Failed: Invalid user credentials"})
		http.Error(w, string(data), http.StatusUnauthorized)
		return
	}

	cbUrl := fmt.Sprintf("http://%s/api/2/saml_assertion/verify_factor", r.Host)

	switch params["username_or_email"] {
	case "samlcodemfa":
		data, _ := json.Marshal(&oneloginSamlReply{
			Message: "MFA is required for this user",
			oneloginAuthData: oneloginAuthData{
				StateToken:  "saml state token",
				CallbackUrl: cbUrl,
				Devices:     []*oneloginMfaDevice{{DeviceType: "Google Authenticator", DeviceId: 123}},
	...
	case "samlpushmfa":
		data, _ := json.Marshal(&oneloginSamlReply{
			...
			oneloginAuthData: oneloginAuthData{
				StateToken:  "saml state token",
				CallbackUrl: cbUrl,
				Devices:     []*oneloginMfaDevice{{DeviceType: "OneLogin Protect", DeviceId: 321}},
	...
	default:
		//nolint:lll
		const samlData = ""
		data, _ := json.Marshal(&oneloginAuthReplyV2{Data: samlData, Message: "success"})
		_, _ = w.Write(data)
	}
}

func olSamlVerifyMfaHandler(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
	body, err := io.ReadAll(r.Body)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	...
	case vr.DeviceId == "555":
		...
		data, _ := json.Marshal(&oneloginApiErrorV2{
...
func olMfaDeviceHandler(w http.ResponseWriter, r *http.Request) {
	...
	// path: /api/2/mfa/users/{userId}/devices
	parts := strings.Split(r.URL.Path, "/")
	if len(parts) < 7 || parts[6] != "devices" {
		http.Error(w, "invalid path", http.StatusBadRequest)
		return
	}

	var reply []*oneloginMfaFactor
	switch parts[5] {
	case "123":
		reply = []*oneloginMfaFactor{
			{Id: "555", Type: "Google Authenticator", Default: true, DisplayName: "Google Authenticator"},
		}
	case "321":
		reply = []*oneloginMfaFactor{
			{Id: "666", Type: "OneLogin", Default: true, DisplayName: "OneLogin Protect"},
		}
	default:
